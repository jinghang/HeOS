...
;中断服务程序
isr_common_stub:
    pusha                   ; pushes edi,ebp,esp,ebx,edx,ecx,eax
    mov ax, ds
    push eax                ; 保存数据段描述符
    ...
    push esp                ; 此时的esp寄存器的之等价于 pt_regs 结构体的指针
    call isr_handler        ; 在C语言里实现
    add esp, 4              ; 清除压入栈参数

    pop ebx                 ; 恢复原来数据段描述符，对应 mov ax, 0x10
    mov ds, bx
    mov es, bx
    mov fs, bx
    mov gs, bx
